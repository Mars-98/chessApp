...
class King(Piece):
    ...
    def move_loc(self, location, piece, tiles):
        ...
        y_V_up = location[0] + 1
        x_V_up = location[1]
        self.add_loc(y_V_up, x_V_up, piece, loc_list, tile)

        # vertical down
        y_V_down = location[0] - 1
        x_V_down = location[1]
        self.add_loc(y_V_down, x_V_down, piece, loc_list, tile)

        # horizontal left
        y_H_left = location[0]
        x_H_left = location[1] + 1
        self.add_loc(y_H_left, x_H_left, piece, loc_list, tile)

        # horizontal right
        y_H_right = location[0]
        x_H_right = location[1] - 1
        self.add_loc(y_H_right, x_H_right, piece, loc_list, tile)
    ...
    def __add_loc(self, x, y, piece, loc_list, tile):
